...
class MastodonSwiftTests: XCTestCase {
    ...
    func testAuthentication() async throws {
        ...
        XCTAssertEqual(response.accessToken, "s3cr3t_t0k3n")
    }
    ...
    func testGetHomeTimeline() async throws {
        ...
        let client = try MastodonClient(baseURL: URL(string: "https://bearologics.social")!, urlSession: session)
            .getAuthenticated(token: token)
        
        let result = try await client.getHomeTimeline()
        
        XCTAssertEqual(result.first?.content.raw, "<p>TEST</p>")
    }
    ...
    func testGetHomeTimeline_withoutConvenienceMethod() async throws {
        ...
        let result = try MastodonClient.jsonDecoder.decode([Status].self, from: data)
